...
class _HomeFormState extends State<HomeForm> {
  ...
  _onSubmited() {
    BlocProvider.of<GetHomeBloc>(context).add(
      GetHomeButtonPressed(    
      ),
    );
  }

  _onClick() {
    BlocProvider.of<CekFiturCoinBloc>(context).add(
      CekFiturCoinButtonPressed(    
      ),
    );
  }
  ...
  setParam(anggota) async{
    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setInt("isAnggota", anggota);
  }

  @override
  Widget build(BuildContext context) {
    return BlocListener<CekFiturCoinBloc, CekFiturCoinState>(
      listener: (context, state) {
        if(state is CekFiturCoinLoading){
          setState(() {
             _loading = true;         
          });
        }
        if(state is CekFiturCoinFailure){
          setState(() {
             _loading = false;         
          });
        ...
        if(state is CekFiturCoinInitial){
          setState(() {
             _loading = false;         
            _listCoin = state.status.response;
          });
          if(_listCoin.data.isPikomigrations == 0 && _listCoin.data.isPikomigrations == 0){

          }else if(_listCoin.data.isPikomigrations == 0 && _listCoin.data.isPikomigrations == 0){
      ...
      child:(_loading == false) ? Center(child: CircularProgressIndicator(),): BlocBuilder<GetHomeBloc, ProfileState>(
        // bloc: _getBloc,
        builder: (context, state) {  
          ...
          if (state is GetHomeFailure) {
            if(state.home.response.respCode =="04"){
              BlocProvider.of<AuthenticationBloc>(context).add(LoggedOut());
              Navigator.pushReplacement(context, MaterialPageRoute(
                builder: (context) => LoginMain()
              ));
          ...
          if(state is GetHomeInitial){
            // print();
            int _anggota = state.home.response.data.isAnggota;
            setParam(_anggota);            
